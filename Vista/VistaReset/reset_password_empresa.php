...
$wrapper = new empresaRecuperacionControladorWrapper($conn);

$token = $_GET['token'] ?? '';
$msg = '';

if (!$token) {
    die('Token no proporcionado. Por favor, solicita un nuevo enlace.');
}

$tokenValido = $wrapper->validarTokenEmpresa($token);
if (!$tokenValido['valido']) {
    die(htmlspecialchars($tokenValido['msg']));
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($password !== $password2) {
        ...
        $res = $wrapper->resetearContrasenaEmpresa($token, $password);
        if (!$res['exito']) {
            $msg = $res['msg'];
        } else {
            $empresa = $wrapper->obtenerEmpresaPorId($tokenValido['datos']['id_empresa']);
            if ($empresa) {
...
<div class="reset-container">
    <h2>Cambiar contraseña — Empresa</h2>
    <?php if($msg): ?>
